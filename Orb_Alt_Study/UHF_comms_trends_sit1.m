...
%% FUNCTION - TYPICAL GROUND USER UHF STUDY
tic
out = struct;
...
save('Output Files\UHF_comms_trends_sit1_out','out','res','ustat','keps',...
     'dt','sols','sit','frq','BW','powt','point','t')

%% POST-PROCESSING
%Load information from file if required 
%load('Output Files\UHF_comms_trends_sit1_out')
...
sgtitle({'Unsteered UHF Comms w/ Electra','Lat=0 Equatorial Ground User to i=0 Orbiter','15W & 2dBi Boresight Gain'})
fig=gcf;
...
if log == 1
[~,~] = pass_over(1,sit,2,frq,BW,powt,elec,keps(row,:),ustat,t_plot,dt_plot,point,...
