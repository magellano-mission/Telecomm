...
orb_alts2 = 200:200:3000;              %[km] altitude range of interest
...
hard = sys_hard(0,0,custt,custr,'sdst',290,[0 3],sit);
...
subplot(2,2,1)
levels =  100:100:1000;
[C,h] = contour(orb_alts2,orb_alts1,plots(:,:,1),levels);
...
subplot(2,2,2)
levels = 100:100:1000;
[C,h] = contour(orb_alts2,orb_alts1,plots(:,:,2)*1000,levels);
clabel(C,h,'manual','BackgroundColor','white','Margin',0.5,'FontSize',10)
h.LineWidth = 2.5;
grid on
ylabel('ECS Altitude [km]')
...
subplot(2,2,3)
levels = 0:1:24;
[C,h] = contour(orb_alts2,orb_alts1,plots(:,:,3),levels);
ylabel('ECS Altitude [km]')
xlabel('RS Orbiter Altitude [km]')
